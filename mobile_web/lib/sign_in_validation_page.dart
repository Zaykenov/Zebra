...
class SignInValidationPage extends StatefulWidget {
  final String email;
  const SignInValidationPage(this.email, {Key? key}) : super(key: key);

  @override
  State<SignInValidationPage> createState() => _SignInValidationPageState();
}

class _SignInValidationPageState extends State<SignInValidationPage> {
              ...
              children: [
                Pinput(
                  ...
                  onCompleted: (emailCode) async {
                    ...
                    var res = await ApiCaller().verifySignInEmailCode(
                        VerifyEmailCodeReq(widget.email, emailCode, deviceId!));
                    await SmartDialog.dismiss();

                    if (res.status ==
                        SignInVerifyEmailCodeResStatus.IncorrectCode) {
                      await SmartDialog.showNotify(
                          msg: "Некорректный код",
                          notifyType: NotifyType.error);
                    }
                    if (res.status ==
                        SignInVerifyEmailCodeResStatus.ToManyAttemps) {
                      SmartDialog.showNotify(
                          msg: "Слишком много попыток. Попробуйте позже",
                          notifyType: NotifyType.error);
                    }
                    if (res.status ==
                        SignInVerifyEmailCodeResStatus.ValidCode) {
                      await SessionHelper()
                          .userSignIn(res.email!, res.name!, res.clientId!);
